...
void setup() {
  ...
  Serial.println("ligado"); //informar quando rodar
...
void TesteComando()              // Teste para mostrar a função de recebimento de dados do esp32
{
  ...
  while( Serial.available() > 0 )  // enviar dados pro esp32 pelo computador
  {
     ...
     if (comandocomputador == 'L'){   // Se inserir L no computador acenderá o led do arduino
     ...
     if (comandocomputador == 'D'){   // Se inserir D no computador desligará o led do arduino
      digitalWrite(led, HIGH);
